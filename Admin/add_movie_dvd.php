...
include_once ("profile_admin.php");
    ...
    if (isset($_POST['name'])){
		...
		$query = "INSERT INTO `dvd` (name, release_date, category, available, type) 
					VALUES ('$name', '$release', '$category', '$available', '$type')";
		$result = mysqli_query($conn, $query);
		if($result==true){
		echo "<script type='text/javascript'>alert('DVD Movie Added!');</script>";
		
		}
		else{
		echo "<script type='text/javascript'>alert('DVD Movie Not Added!')</script>";
		}
